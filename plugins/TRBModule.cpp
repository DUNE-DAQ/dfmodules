...
TRBModule::generate_opmon_data()
{
  ...
  i.set_received_fragments(m_received_fragments.exchange(0));
  ...
  i.set_td_processing_us(m_td_processing_us.exchange(0));
  i.set_fragment_processing_us(m_fragment_processing_us.exchange(0));
  ...
  // system behavior like trigger inhibits.
  {
    ...
    for (const auto& sid_sender : m_map_sourceid_connections) {
      ...
      if (sender != nullptr) {
        ...
        TLOG_DEBUG(0) << "The DataRequest sender for " << sid_sender.first << " " << (is_ready ? "is" : "is not")
                      << " ready.";
      }
    }
  }
...
TRBModule::fragments_callback(std::unique_ptr<daqdataformats::Fragment>& temp_fragment)
{
  ...
  TLOG_DEBUG(TLVL_FRAGMENT_RECEIVE) << get_name() << " Received fragment for trigger/sequence_number "
                                    << temp_fragment->get_trigger_number() << "."
  ...
  { // Begin mutex block
    ...
    // Only do bookkeeping periodically
    if (std::chrono::duration_cast<std::chrono::milliseconds>(clock_type::now() - m_last_bookkeeping).count() > 1000) {
      //-------------------------------------------------
  ...
  m_fragment_processing_us += std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
}
...
TRBModule::extract_trigger_record(const dfmessages::TriggerId& id)
{
  ...
  trigger_record_ptr_t temp = std::move(it.mapped().second);

  auto time = clock_type::now();
  auto duration = time - it.mapped().first;
...
bool
TRBModule::dispatch_data_requests(dfmessages::DataRequest dr, const daqdataformats::SourceID& sid)
