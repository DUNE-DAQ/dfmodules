...
enum { TLVL_ENTER_EXIT_METHODS = 5, TLVL_CONFIG = 7, TLVL_WORK_STEPS = 10 };

namespace dunedaq {
namespace dfmodules {

DataFlowOrchestrator::DataFlowOrchestrator(const std::string &name)
    : dunedaq::appfwk::DAQModule(name), m_queue_timeout(100), m_run_number(0),
      m_working_thread(std::bind(&DataFlowOrchestrator::do_work, this,
                                 std::placeholders::_1)) {
...
void DataFlowOrchestrator::init(const data_t &init_data) {
...
void DataFlowOrchestrator::do_conf(const data_t &payload) {
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS)
      << get_name() << ": Entering do_conf() method";

  datafloworchestrator::ConfParams parsed_conf =
      payload.get<datafloworchestrator::ConfParams>();

  for (auto &app : parsed_conf.dataflow_applications) {
    m_dataflow_availability[app.decision_connection] =
        TriggerRecordBuilderData(app.decision_connection, app.capacity);
  }
...
void DataFlowOrchestrator::do_start(const data_t &payload) {
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS)
  ...
  networkmanager::NetworkManager::get().register_callback(
      m_token_connection_name,
      std::bind(&DataFlowOrchestrator::receive_trigger_complete_token, this,
                std::placeholders::_1));
...
void DataFlowOrchestrator::do_stop(const data_t & /*args*/) {
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS)
...
void DataFlowOrchestrator::do_scrap(const data_t & /*args*/) {
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS)
      << get_name() << ": Entering do_scrap() method";

  networkmanager::NetworkManager::get().stop_listening(m_token_connection_name);

  TLOG() << get_name() << " successfully stopped";
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS)
      << get_name() << ": Exiting do_scrap() method";
}

void DataFlowOrchestrator::do_work(std::atomic<bool> &run_flag) {
      ...
      if (res) {
        ...
        m_waiting_for_decision +=
            std::chrono::duration_cast<std::chrono::microseconds>(
                assignment_possible - slot_available)
                .count();
        ...
        while (run_flag.load()) {
          ...
          if (dispatch_successful) {
            ...
            ers::error(TriggerRecordBuilderAppUpdate(
                ERS_HERE, assignment->connection_name,
                "Could not send Trigger Decision"));
            m_dataflow_availability[assignment->connection_name].set_in_error(
                true);
          }
        }
        ...
        m_deciding_destination +=
            std::chrono::duration_cast<std::chrono::microseconds>(
                assignment_complete - assignment_possible)
                .count();

      } else { // failed at extracting the decisions
        m_waiting_for_decision +=
            std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - slot_available)
                .count();
      }

    } else {
      auto lk = std::unique_lock<std::mutex>(m_slot_available_mutex);
      m_slot_available_cv.wait_for(lk, std::chrono::milliseconds(1),
                                   [&]() { return has_slot(); });
      auto new_time_check = std::chrono::steady_clock::now();
      m_waiting_for_slots +=
          std::chrono::duration_cast<std::chrono::microseconds>(new_time_check -
                                                                last_slot_check)
              .count();
...
DataFlowOrchestrator::find_slot(dfmessages::TriggerDecision decision) {
  static std::map<std::string, TriggerRecordBuilderData>::iterator
...
void DataFlowOrchestrator::get_info(opmonlib::InfoCollector &ci,
                                    int /*level*/) {
...
void DataFlowOrchestrator::receive_trigger_complete_token(
    ipm::Receiver::Response message) {
  auto token = serialization::deserialize<dfmessages::TriggerDecisionToken>(
      message.data);
  ++m_received_tokens;

  if (token.run_number == m_run_number) {
    m_dataflow_availability[token.decision_destination].complete_assignment(
        token.trigger_number, m_metadata_function);

    if (m_dataflow_availability[token.decision_destination].is_in_error()) {
      TLOG() << TriggerRecordBuilderAppUpdate(
          ERS_HERE, token.decision_destination, "Has reconnected");
...
bool DataFlowOrchestrator::has_slot() const {
...
bool DataFlowOrchestrator::extract_a_decision(
    dfmessages::TriggerDecision &decision) {
  ...
  try {
    ...
    TLOG_DEBUG(TLVL_WORK_STEPS)
        << get_name() << ": Popped the Trigger Decision with number "
...
bool DataFlowOrchestrator::dispatch(
    std::shared_ptr<AssignedTriggerDecision> assignment,
    std::atomic<bool> &run_flag) {

  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS)
      << get_name() << ": Entering dispatch() method";
  auto serialised_decision = dunedaq::serialization::serialize(
      assignment->decision, dunedaq::serialization::kMsgPack);
  ...
  do {
    ...
    try {
      networkmanager::NetworkManager::get().send_to(
          ...
          serialised_decision.size(), m_queue_timeout);
      wasSentSuccessfully = true;
      ++m_sent_decisions;
    } catch (const ers::Issue &excpt) {
      std::ostringstream oss_warn;
      oss_warn << "Send to connection \"" << assignment->connection_name
               << "\" failed";
      ers::warning(
          networkmanager::OperationFailed(ERS_HERE, oss_warn.str(), excpt));
    }
  ...
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS)
      << get_name() << ": Exiting dispatch() method";
  return wasSentSuccessfully;
}

void DataFlowOrchestrator::assign_trigger_decision(
    std::shared_ptr<AssignedTriggerDecision> assignment) {
  m_dataflow_availability[assignment->connection_name].add_assignment(
      assignment);
}
