...
#include "appmodel/FilterWriterConf.hpp"
...
/**
 * @brief FilterWriterModule is a shell for what we might write for the MiniDAQApp.
 */
class FilterWriterModule : public dunedaq::appfwk::DAQModule
{
public:
  /**
   * @brief FilterWriterModule Constructor
   * @param name Instance name for this FilterWriterModule instance
   */
  explicit FilterWriterModule(const std::string& name);

  FilterWriterModule(const FilterWriterModule&) = delete;            ///< FilterWriterModule is not copy-constructible
  FilterWriterModule& operator=(const FilterWriterModule&) = delete; ///< FilterWriterModule is not copy-assignable
  FilterWriterModule(FilterWriterModule&&) = delete;                 ///< FilterWriterModule is not move-constructible
  FilterWriterModule& operator=(FilterWriterModule&&) = delete;      ///< FilterWriterModule is not move-assignable
...
private:
  ...
  const appmodel::FilterWriterConf* m_filter_writer_conf;
  std::string m_writer_identifier;
  std::shared_ptr<FilterOrchestratorClient> m_client;
  void open_run(daqdataformats::run_number_t run);
  void close_run();
